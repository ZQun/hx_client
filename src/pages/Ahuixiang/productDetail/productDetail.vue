<template>
	<view class="container">
		...
		<view class="tui-pro-detail">
			<view class="tui-product-title tui-border-radius">
				<view class="tui-pro-pricebox tui-padding">
					<view class="tui-pro-price">
						...
						<view v-if="!vip_status">￥
							<text class="tui-price">{{Number(goodsInfo.price) / 100}}</text>
						</view>
						<view v-else>VIP
							<text class="tui-price">{{Number(goodsInfo.vip_price) / 100}}</text>
						</view>
				...
				<view v-else class="tui-original-price tui-gray">
					价格
					<text class="tui-line-through">￥{{Number(goodsInfo.price) / 100}}</text>
				</view>
				<view class="tui-pro-titbox">
					<view class="tui-pro-title">{{goodsInfo.title}}</view>
				...
				<view class="tui-padding">
					<view class="tui-sub-title tui-size tui-gray">{{goodsInfo.summary}}</view>
					<view class="tui-sale-info tui-size tui-gray">
						<view>快递：{{Number(goodsInfo.express_price) / 100}}</view>
						<view>月销{{goodsInfo.sales_num}}</view>
						<view v-if="goodsInfo.hx_merchant">{{goodsInfo.hx_merchant.province + ' ' + goodsInfo.hx_merchant.city}}</view>
					</view>
				</view>
			</view>
			...
			<view class="tui-cmt-box tui-mtop tui-radius-all" v-if="goodsInfo.hx_merchant">

				<view class="tui-cmt-content tui-padding" style="font-size: 40rpx; margin-top: 36rpx;">
					<view class="tui-cmt-user">
						<image :src="goodsInfo.hx_merchant.logo_url" style="width: 100rpx; height: 100rpx; border-radius: 0; border: solid 1px #e9e9e9;" class="tui-acatar"></image>
						<view>{{goodsInfo.hx_merchant.title}}</view>
					</view>
				</view>

				<view class="tui-cmt-btn-sel">
					<tui-tag type="black" :plain="true" tui-tag-class="tui-tag-cmt" @tap="homepath(goodsInfo.hx_merchant.id)">查看店铺主页</tui-tag>
				</view>
			</view>
			...
			<view class="tui-product-img tui-radius-all">
				...
				<u-parse :content="goodsInfo.content_text" @preview="preview" @navigate="navigate" />
			</view>
		...
		<view class="tui-operation">
			...
			<view class="tui-operation-right tui-right-flex tui-col-7 tui-btnbox-4">
				<view class="tui-flex-1">
					<tui-button type="warning" shape="circle" size="mini" @click="showPopup">加入购物车</tui-button>
				</view>
				<view class="tui-flex-1">
			...
			<view class="tui-popup-box">
				<view class="tui-product-box tui-padding">
					...
					<view class="tui-popup-price">
						<view v-if="!vip_status" class="tui-amount tui-bold">￥{{Number(goodsInfo.price) / 100}}</view>
						<view v-else class="tui-amount tui-bold">VIP {{Number(goodsInfo.vip_price) / 100}}</view>
						<view class="tui-number">编号: {{goodsInfo.id}}</view>
					</view>
				</view>
...
export default {
	...
	onLoad: function (options) {
		...
		if (this.id !== undefined) {
			// 请求商品数据
			console.log('->请求商品数据', this.id)
			this.getGoodsInfo(Number(this.id));
		}
	...
	onShareAppMessage(res) {
		...
		let path = '/pages/Ahuixiang/productDetail/productDetail'
		...
		try {
			...
			if (value) {
				...
				return { title, path: `${path}?path=${Base64.encode(id)}&id=${this.id}${str}` }
			} else {
				return { title, path: `${path}?id=${this.id}${str}` }
			}
		} catch (e) {
			return { title, path: `${path}?id=${this.id}${str}` }
		}
	...
	methods: {
		...
		common: function () {
			...
			// goodsInfo
			uni.navigateTo({
				url: `/pages/Ahx-extend/comment/productComment?id=${this.goodsInfo.id}`,
				animationType: 'pop-in',
				animationDuration: 200
			});
		},
		...
		getGoodsInfo(id) {
			uni.request({
				url: `${getApp().globalData.URL}/clientGoods/info?id=${id}`,
				success: (res) => {
					this.banner = res.data !== '暂无数据' ? JSON.parse(res.data.img_url) : [];
