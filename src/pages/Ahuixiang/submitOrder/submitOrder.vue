		...
		<view class="tui-box">
			...
			<view class="tui-top tui-goods-info">
				...
				<block v-if="goodsInfo && !cart_status">
					<tui-list-cell :hover="false" padding="0">
						<view class="tui-goods-item">
							<image :src="JSON.parse(goodsInfo.img_url)[0]" class="tui-goods-img"></image>
							<view class="tui-goods-center">
								<view class="tui-goods-name">{{goodsInfo.title}}</view>
								<!-- <view class="tui-goods-attr">黑色，50ml</view> -->
							</view>
							<view class="tui-price-right">
								<view v-if="!vip_status">￥{{Number(goodsInfo.price) / 100}}</view>
				...
				<tui-list-cell v-else :hover="false">
					<view class="tui-padding tui-flex">
						<view>商品总额</view>
						<view v-if="!vip_status">￥{{Number(goodsInfo.price * size) / 100}}</view>
				...
				<tui-list-cell :hover="false">
					<view class="tui-padding tui-flex">
						...
						<view v-else>￥{{Number(goodsInfo.express_price / 100)}}</view>
					</view>
				</tui-list-cell>
...
<script>
	...
	export default {
		...
		computed: {
			...
			countPrice: function() {
				// 购物车 false && !false => true
				if(this.cart_status && !this.vip_status) {
					let countPrice = 0;
					this.goodsCart.forEach(item => {
						countPrice = countPrice + item.hx_goods.express_price + (item.num * item.hx_goods.price);
					})
					return countPrice / 100
				} 
				if (this.cart_status && this.vip_status) {
					...
					this.goodsCart.forEach(item => {
						// 如果该商品没有VIP价格，则以原价计算
						if(item.hx_goods.vip_price) {
							countPrice = countPrice + item.hx_goods.express_price + (item.num * item.hx_goods.vip_price);
						} else {
							countPrice = countPrice + item.hx_goods.express_price + (item.num * item.hx_goods.price);
						}
					})
				...
				if(this.vip_status && !this.cart_status) {
					return Number(this.goodsInfo.express_price / 100) + (Number(this.goodsInfo.vip_price * this.size) / 100)
				}
				return Number(this.goodsInfo.express_price / 100) + (Number(this.goodsInfo.price * this.size) / 100)
			},
		...
		onLoad(options) {
			...
			this.size = options.num ? Number(options.num) : 0;
			...
			// 购物车进入
			if(options.cart === 'cart') {
				this.cart_status = true;
				this.cart_id = options.arr + '&cart=cart'
				this.getCartList(this.cart_id);
			}
				...
				if(this.userInfo.hx_user_address.length === 0) {
					...
					uni.getStorage({
						...
						success: (res) => {
							...
							uni.request({
								url: `${getApp().globalData.URL}/clientOrder/submitOrder`,
								method: 'POST',
								header: {
											...
											success: function (res) {
												uni.requestPayment({
													...
													fail: (fail) => {
														...
														if(thls.cart_status) {
															// 查询商品信息
															thls.getCartList(thls.cart_id);
														} else {
															// 查询商品信息
															thls.getGoodsInfo(thls.id);
														}
														thls.getUserInfo();
													}
												})
