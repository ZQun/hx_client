				...
				<view class="tui-new-item" :class="[index!=0 && index!=1 ?'tui-new-mtop':'']" v-for="(item,index) in newProduct" :key="index" @tap="detail(item.id)">
					...
					<view class="tui-title-box">
						...
						<view class="tui-new-price">
							<text class="tui-new-present">VIP {{Number(item.vip_price) / 100}}</text>
							<text class="tui-new-original">￥{{Number(item.price) / 100}}</text>
						</view>
					</view>
							...
							<view class="tui-pro-content">
								...
								<view>
									<view class="tui-pro-price">
										<text class="tui-sale-price">VIP {{Number(item.vip_price) / 100}}</text>
										<text class="tui-factory-price">￥{{Number(item.price) / 100}}</text>
									</view>
...
export default {
	...
	methods: {
		...
		more: function (e, id) {
			...
			uni.navigateTo({
				url: `../vip/productList?searchKey=${key}&type=type&type_id=${id}`
			})
		},
		moreMerchant: function () {
			uni.navigateTo({
				url: `../vip/productList?searchKey=平台自营&type=merchant&state=vip`
			})
		},
		search: function () {
			uni.navigateTo({
		...
		getInfo() {
			uni.request({
				url: `${getApp().globalData.URL}/getSetting/getInfo?vip=vip`,
				success: (res) => {
					this.newsList = res.data.affiche ? res.data.affiche : [{ title: '暂无公告' }];
					this.banner = res.data.image ? res.data.image : [];
		...
		getGoodsListPage(page) {
			...
			uni.request({
				url: `${getApp().globalData.URL}/clientGoods/page?page=${page}&vip=vip`,
				success: (res) => {
		...
		getGoodsMerchantList() {
			// getGoodsList
			uni.request({
				url: `${getApp().globalData.URL}/clientGoods/getGoodsList?vip=vip`,
				success: (res) => {
	...
	onShareAppMessage(res) {
		// 分享带分享者id
		let title = '慧享优选VIP专区'
		let path = '/pages/Ahx-extend/vip/vip'
