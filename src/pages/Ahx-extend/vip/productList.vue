			...
			<view class="tui-product-container">
				<block v-for="(item,index) in goodsList" :key="index" v-if="(index+1)%2!=0 || isList">
					...
					<view class="tui-pro-item" :class="[isList?'tui-flex-list':'']" hover-class="hover" :hover-start-time="150" @tap="detail(item.id)">
						<image :src="JSON.parse(item.img_url)[0]" class="tui-pro-img" :class="[isList?'tui-proimg-list':'']" mode="widthFix" />
						<view class="tui-pro-content">
							<view class="tui-pro-tit">{{item.title}}</view>
							<view>
								<view class="tui-pro-price">
									<text class="tui-sale-price">VIP￥{{item.vip_price / 100}}</text>
									<text class="tui-factory-price">￥{{item.original_price / 100}}</text>
								</view>
...
export default {
	...
	onShareAppMessage(res) {
		...
		let path = '/pages/Ahx-extend/vip/productList'

		let basePath = {
			'type': `searchKey=${this.searchKey}&type=${this.goodsType}&type_id=${this.type_id}`,
			'merchant': `searchKey=${this.searchKey}&type=${this.goodsType}&state=vip`,
			'search': `searchKey=${this.searchKey}&type=${this.goodsType}&state=vip`,
		}
