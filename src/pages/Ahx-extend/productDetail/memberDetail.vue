<template>
	<view class="container">
		...
		<view class="tui-pro-detail">
			<view class="tui-product-title tui-border-radius">
				<view class="tui-pro-pricebox tui-padding">
					<view class="tui-pro-price">
						...
						<view v-if="!vip_status">￥<text class="tui-price">{{Number(goodsInfo[0].price) / 100}}</text></view>
						<!-- <view v-else>VIP <text class="tui-price">{{Number(goodsInfo.vip_price) / 100}}</text></view> -->
				...
				<view class="tui-pro-titbox">
					<view class="tui-pro-title">{{goodsInfo[0].name}}</view>
			...
			<view class="tui-basic-info tui-mtop tui-radius-all">
				<view class="tui-list-cell" @tap="showPopup">
					...
					<view class="tui-selected-box">编号:【{{goodsInfo[0].name}}】，{{value}}个</view>
		...
		<view class="tui-operation">
			...
			<!-- </view> -->
			<view style="width: 100%" class="tui-right-flex tui-btnbox-4">
				<!-- <view class="tui-flex-1">
					<tui-button type="warning" shape="circle" size="mini" @click="showPopup">加入购物车</tui-button>
				</view> -->
				<view class="tui-flex-1">
					<tui-button type="green" shape="circle" size="mini" @click="showPopup">立即购买</tui-button>
				</view>
			</view>
		</view>
			...
			<view class="tui-popup-box">
				<view class="tui-product-box tui-padding">
					...
					<view class="tui-popup-price">
						<view v-if="!vip_status" class="tui-amount tui-bold">￥{{selectPrice}}</view>
						<!-- <view v-else class="tui-amount tui-bold">VIP {{Number(goodsInfo.vip_price) / 100}}</view> -->
						<view class="tui-number">编号: {{goodsInfo[0].id}}</view>
					</view>
				</view>
				<scroll-view scroll-y class="tui-popup-scroll">
					<view class="tui-scrollview-box">
						...
						<view class="tui-attr-box">
							<view v-for="(item, index) in goodsInfo" :key="index" @tap="onClickType(index)" :class="classIndex === index ? 'tui-attr-item tui-attr-active' : 'tui-attr-item'">
								{{item.name}}/赠送{{item.integral}}阳光币
							</view>
							<!-- <view class="tui-attr-item tui-attr-active">
								18cm
							</view> -->
	...
	export default {
		...
		computed: {
			selectPrice: function() {
				return this.goodsInfo[this.classIndex].price / 100
			}
		},
		methods: {
			onClickType(index) {
					...
					success: (res) => {
						...
						uni.getStorage({
							...
							success: (res) => {
								...
								uni.request({
									url: `${getApp().globalData.URL}/clientOrder/submitMemberOrder`,
									method: 'POST',
									header: {
										'Authorization': "Bearer " + this.token.token
									},
									data: {
										id: this.goodsInfo[this.classIndex].id,
										num: this.value,
										path
									},
									success: (res) => {
			...
			getGoodsInfo(id) {
				uni.request({
					url: `${getApp().globalData.URL}/clientUser/getMemberList`,
					success: (res) => {
						if(res.data !== '暂无数据') {
							this.goodsInfo = res.data;
							res.data.forEach(item => {
								this.banner.push(item.img_url)
							})
						}
					}
				})
