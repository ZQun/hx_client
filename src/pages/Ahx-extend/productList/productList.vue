	...
	<view class="container">
		...
		<view class="tui-header-screen" :style="{top:height+'px'}">
			<view class="tui-screen-top">
				...
				<view class="tui-dropdownlist" :class="[selectH>0?'tui-dropdownlist-show':'']" :style="{height:selectH+'rpx'}">
					<view class="tui-dropdownlist-item tui-icon-middle" :class="[item.selected?'tui-bold':'']" v-for="(item,index) in dropdownList" :key="index" @tap.stop="dropdownItem" :data-index="index">
			...
			<!-- <view class="tui-screen-bottom">
				<block v-for="(item,index) in attrArr" :key="index">
					<view class="tui-bottom-item tui-icon-ml" :class="[item.isActive?'tui-btmItem-active':'',attrIndex==index?'tui-btmItem-tap':'']" :data-index="index" @tap="btnDropChange">
						<view class="tui-bottom-text" :class="[attrIndex==index?'tui-active':'']">{{item.isActive?item.selectedName:item.name}}</view>
						<tui-icon :name="attrIndex==index?'arrowup':'arrowdown'" :size="14" :color="attrIndex==index || item.isActive?'#e41f19':'#444'" tui-icon-class="tui-ml" v-if="item.list.length>0"></tui-icon>
					</view>
				</block>
			</view> -->
		</view>
		...
		<view class="tui-product-list" :style="{marginTop:px(dropScreenH+18)}">
			<view class="tui-product-container">
				<block v-for="(item,index) in goodsList" :key="index" v-if="(index+1)%2!=0 || isList">
					...
					<view class="tui-pro-item" :class="[isList?'tui-flex-list':'']" hover-class="hover" :hover-start-time="150" @tap="detail(item.id)">
						<image :src="imageURL(item)" class="tui-pro-img" :class="[isList?'tui-proimg-list':'']" mode="widthFix" />
						<view class="tui-pro-content">
							<view class="tui-pro-tit">{{item.hx_goods.title}}</view>
							<view>
								<view class="tui-pro-price">
									<text class="tui-sale-price">{{priceText(item).price}}</text>
									<text class="tui-factory-price">{{priceText(item).original_price}}</text>
								</view>
								<view v-if="goodsType == 'time'" class="tui-pro-pay">{{times(item).text}}</view>
								<tui-countdown v-if="goodsType == 'time'" :time="times(item).time" color="#fff" bcolor="#e41f19" bgcolor="#e41f19" colonColor="#e41f19" @end="endOfTime"></tui-countdown>
								<view v-if="goodsType == 'time'" class="tui-pro-pay">只剩{{priceText(item).stock_num}}件</view>
								<view v-if="goodsType == 'fight'" class="tui-pro-pay">最低{{priceText(item).fight_num}}人成团</view>
								<view v-if="goodsType != 'time'" class="tui-pro-pay">{{priceText(item).sales_num}}人付款</view>
							</view>
						</view>
					</view>
					<!--商品列表-->
		...
		<!-- <tui-top-dropdown bgcolor="#f7f7f7" :show="dropScreenShow" :paddingbtm="110" :translatey="dropScreenH" @close="btnCloseDrop">
			<scroll-view class="tui-scroll-box" scroll-y :scroll-top="scrollTop">
				...
				<view class="tui-drop-item tui-icon-middle" :class="[item.selected?'tui-bold':'']" v-for="(item,index) in attrData" :key="index" @tap.stop="btnSelected" :data-index="index">
					<tui-icon name="check" :size="16" color="#e41f19" :bold="true" v-if="item.selected" tui-icon-class="tui-middle"></tui-icon>
					<text class="tui-ml tui-middle">{{item.name}}</text>
				</view>
			...
			<view class="tui-drawer-box" :style="{paddingTop:height+'px'}">
				<scroll-view class="tui-drawer-scroll" scroll-y :style="{height:drawerH+'px'}">
					...
					<view class="tui-drawer-content">
						<input v-model="minPrice" placeholder-class="tui-phcolor" class="tui-input" placeholder="最低价" maxlength="11" type='number' />
						<tui-icon name="reduce" color="#333" :size="14"></tui-icon>
						<input v-model="maxPrice" placeholder-class="tui-phcolor" class="tui-input" placeholder="最高价" maxlength="11" type='number' />
					</view>
					...
					<view class="tui-drawer-content tui-flex-attr">
						<view v-for="(item, index) in goodsTypeList" :key="index" @tap="selectType(index)" :class="'tui-attr-item ' + activeType(index)">
				...
				<view class="tui-attr-btnbox">
					<view class="tui-attr-safearea">
						<view @tap="cancelDrawer" class="tui-drawer-btn tui-drawerbtn-black" hover-class="tui-white-hover" :hover-stay-time="150">重置</view>
...
export default {
	...
	onLoad: function (options) {
		...
		if (options.type_id) {
			this.type_id = Number(options.type_id)
		}
		...
		uni.getSystemInfo({
			success: (res) => {
				...
				//略小，避免误差带来的影响
				this.dropScreenH = (this.height * 750 / res.windowWidth + 186) - 100;
	...
	onShareAppMessage(res) {
		...
		let path = '/pages/Ahx-extend/productList/productList'

		let basePath = {
			'type': `searchKey=${this.searchKey}&type=${this.goodsType}&type_id=${this.type_id}`,
			'merchant': `searchKey=${this.searchKey}&type=${this.goodsType}`,
			'search': `searchKey=${this.searchKey}&type=${this.goodsType}`,
			'bargain': `searchKey=${this.searchKey}&type=${this.goodsType}`,
			'fight': `searchKey=${this.searchKey}&type=${this.goodsType}`,
			'time': `searchKey=${this.searchKey}&type=${this.goodsType}`,
			'points': `searchKey=${this.searchKey}&type=${this.goodsType}`,
		}
		...
		try {
			...
			if (value) {
				...
				return { title, path: `${path}?path=${Base64.encode(id)}&${basePath[this.goodsType]}` }
			} else {
				return { title, path: `${path}?${basePath[this.goodsType]}` }
			}
		} catch (e) {
			return { title, path: `${path}?${basePath[this.goodsType]}` }
		}
	...
	computed: {
		activeType: function () {
			return function (index) {
				if (this.activeTypeArr.indexOf(index) != -1) {
					return 'tui-btmItem-active'
				}
				return ''
			}
		},
		priceText: function () {
			return function (item) {
				let model = {
					'bargain': {
						price: `可砍至¥${item.end_price / 100}`,
						original_price: `¥${item.start_price / 100}`,
						sales_num: item.hx_goods.sales_num
					},
					'fight': {
						price: `拼团价¥${item.price / 100}`,
						original_price: `¥${item.original_price / 100}`,
						sales_num: item.hx_goods.sales_num,
						fight_num: item.fight_num
					},
					'time': {
						price: `秒杀价¥${item.price / 100}`,
						original_price: `¥${item.original_price / 100}`,
						stock_num: item.stock_num,
					},
					'points': {
						price: `阳光币：${item.points}`,
						original_price: `¥${item.hx_goods.original_price / 100}`,
						sales_num: item.hx_goods.sales_num
					}
				}
				return model[this.goodsType]
			}
		},
		times: function () {
			return function (item) {
	...
	methods: {
		...
		imageURL(item) {
			return JSON.parse(item.hx_goods.img_url)[0]
		},
		selectType(index) {
			let is = this.activeTypeArr.indexOf(index);
			if (is != -1) {
				this.activeTypeArr.splice(is, 1);
			} else {
				this.activeTypeArr.push(index)
			}
		},
		getGoodsListPage(page, type) {
			if (this.pageIndex == 1) {
				this.loadding = true
				this.pullUpOn = true
				this.goodsList = []
			...
			if (this.typeData.length != 0 || type) {
				data.activeType = type;
			}

			uni.request({
				url: `${getApp().globalData.URL}/clientGoods/getUserGoodsList?page=${page}`,
				method: 'POST',
				data,
				success: (res) => {
					// SUCCESS
					this.goodsList = res.data.data ? this.goodsList.concat(res.data.data) : [];
		...
		getGoodsTypeAll() {
			uni.request({
				url: `${getApp().globalData.URL}/clientGoods/getGoodsTypeAll`,
				method: 'GET',
				success: (res) => {
					this.goodsTypeList = res.data !== '暂无数据' ? res.data : []
					if (this.type_id) {
		...
		closeDrawer: function () {
			...
			if ((this.minPrice != '' && this.maxPrice == '') || (this.minPrice == '' && this.maxPrice != '')) {
				uni.showToast({ icon: 'none', title: '请输入价格区间', duration: 2000 });
				return
			}
			// 分类数据
			this.activeTypeArr.forEach((item, index) => {
				this.typeData.push(this.goodsTypeList[item].id)
			})

			console.log('->type  id', this.typeData)

			this.drawer = false

			this.pageIndex = 1;
			this.getGoodsListPage(this.pageIndex, this.typeData);
		},
		cancelDrawer() {
			this.minPrice = ''
			this.maxPrice = ''
		...
		search: function () {
			uni.navigateTo({
				url: `../exercise/search.exercise?type=${this.goodsType}`
			})
		},
		detail: function (id) {
			let path = {
				'bargain': `../exercise/productDetail?id=${id}&type=hx_exercise_bargain`,
				'fight': `../exercise/productDetail.fight?id=${id}&type=hx_exercise_fight`,
				'time': `../exercise/productDetail.time?id=${id}&type=hx_exercise_time_limit`,
				'points': `../exercise/productDetail.points?id=${id}&type=hx_exercise_points`
			}
			uni.navigateTo({ url: path[this.goodsType] })
		},
	},
	//页面相关事件处理函数--监听用户下拉动作
