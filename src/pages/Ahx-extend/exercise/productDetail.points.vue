	...
	<view class="container">
			...
			<view class="tui-product-title tui-border-radius">
				<view class="tui-pro-pricebox tui-padding">
					<view class="tui-pro-price">
						...
						<view v-if="true">阳光币<text class="tui-price">{{Number(goodsInfo.points)}}</text></view>
						...
						<tui-tag size="small" :plain="true" type="high-green" shape="circle">阳光币商品</tui-tag>
				...
				<view v-if="!vip_status" class="tui-original-price tui-gray">
					原价
					<text class="tui-line-through">￥{{Number(goodsInfo.hx_goods.original_price) / 100}}</text>
				</view>
		...
		<view class="tui-operation">
			...
			<view class="tui-operation-right tui-right-flex tui-col-7 tui-btnbox-4">
				...
				<view class="tui-flex-1">
					<tui-button v-if="!goodsInfo.getUser" :disabled="!times(goodsInfo).status" type="green" shape="circle" size="mini" @click="submitBargain">立即兑换</tui-button>
...
<script>
	...
	export default {
		...
		onLoad: function(options) {
			...
			if(this.id !== undefined && options.type === 'hx_exercise_points') {
		...
		onShareAppMessage(res) {
			...
			// 	console.log('--uid', uid)
			// 	return {
			// 		title: '阳光币就可兑换，快来兑换吧！' + '[' + this.goodsInfo.hx_goods.title + ']',
			// 		path: `/pages/Ahx-extend/exercise/productDetail.time?id=${id}&uid=${uid}&type=hx_exercise_points`
			// 	}
			// }

			// 分享带分享者id
			let title = '阳光币就可兑换，快来兑换吧！' + '[' + this.goodsInfo.hx_goods.title + ']'
			let path = '/pages/Ahx-extend/exercise/productDetail.points'

			try {
				const value = uni.getStorageSync('token');
				if (value) {
					let id = JSON.parse(value).id;
					return { title, path: `${path}?path=${Base64.encode(id)}&id=${this.id}&type=hx_exercise_points` }
				} else {
					return { title, path: `${path}?id=${this.id}&type=hx_exercise_points` }
				}
			} catch (e) {
				return { title, path: `${path}?id=${this.id}&type=hx_exercise_points` }
			}
				...
				uni.getUserInfo({
					...
					success: (res) => {
						...
						uni.getStorage({
							...
							success: (res) => {
								this.token = JSON.parse(res.data);
								let url = `../submitOrder/submitOrder.points?id=${this.id}&type=${this.type}`
								uni.navigateTo({ url })
							},
							fail: (fail) => {
								uni.navigateTo({
					...
					success: (res) => {
						...
						uni.getStorage({
							...
							success: (res) => {
								this.token = JSON.parse(res.data);
								uni.request({
									url: `${getApp().globalData.URL}/clientExercise/submitPoints?id=${this.id}`,
									method: 'POST',
									header: {
										'Authorization': "Bearer " + this.token.token
									},
					...
					success: (res) => {
						...
						uni.getStorage({
							...
							success: (res) => {
								...
								uni.request({
									url: `${getApp().globalData.URL}/clientExercise/getBargainInfoPoints?id=${id}&type=${type}`,
									method: 'GET',
									header: {
										'Authorization': "Bearer " + this.token.token
									},
