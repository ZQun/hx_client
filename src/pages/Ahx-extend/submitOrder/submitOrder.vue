	...
	<view class="container">
		<view class="tui-box">
			...
			<view class="tui-top tui-goods-info">
					...
					<tui-list-cell :hover="false" padding="0">
						<view class="tui-goods-item">
							...
							<view class="tui-price-right">
								<view v-if="!vip_status">￥{{price}}</view>
								<view>x{{size}}</view>
							</view>
						</view>
					</tui-list-cell>
				...
				<tui-list-cell v-else :hover="false">
					<view class="tui-padding tui-flex">
						<view>商品总额</view>
						<view v-if="!vip_status">￥{{price * size}}</view>
						<!-- <view v-else>￥{{Number(goodsInfo.hx_fight_user[0].price * size) / 100}}</view> -->
		...
		<view class="tui-tabbar">
			<view class="tui-flex-end tui-color-red tui-pr-20">
				...
				<view class="tui-size-26">￥</view>
				<view class="tui-price-large">{{factPayPrice}}</view>
				<!-- <view class="tui-size-26">.00</view> -->
			</view>
			<view class="tui-pr25">
...
<script>
	...
	export default {
		...
		computed: {
			price: function() {
				if(this.type === 'hx_exercise_fight') {
					return Number(this.goodsInfo.hx_fight_user[0].price) / 100
				}
				if(this.type === 'hx_exercise_time_limit') {
					return Number(this.goodsInfo.price) / 100
				}
				return Number(this.goodsInfo.hx_bargain_user[0].price) / 100
			},
			cartPrice: function () {
				let countPrice = 0;
			...
			countPrice: function() {
				if(this.type === 'hx_exercise_fight') {
					return Number(this.goodsInfo.hx_goods.express_price / 100) + (Number(this.goodsInfo.hx_fight_user[0].price * this.size) / 100)
				}
				if(this.type === 'hx_exercise_time_limit') {
					return Number(this.goodsInfo.hx_goods.express_price / 100) + (Number(this.goodsInfo.price * this.size) / 100)
				}
				return Number(this.goodsInfo.hx_goods.express_price / 100) + (Number(this.goodsInfo.hx_bargain_user[0].price * this.size) / 100)
			},
		...
		onLoad(options) {
			...
			if(this.id) {
				if(options.uid !== undefined) {
					...
					this.getGoodsInfo(this.id, this.type, this.uid);
				} else {
					// 查询商品信息
		...
		methods: {
				...
				if(this.userInfo.hx_user_address.length === 0) {
					...
					uni.getStorage({
						...
						success: (res) => {
							...
							uni.request({
								url: `${getApp().globalData.URL}/clientExercise/submitBargainOrder`,
								method: 'POST',
								header: {
											...
											success: function (res) {
												uni.requestPayment({
													...
													fail: (fail) => {
														...
														thls.getGoodsInfo(thls.id, thls.type);
														thls.getUserInfo();
													}
												})
											}
			...
			getGoodsInfo(id, type, uid) {
				let path = 'getBargainInfo'
				if(type === 'hx_exercise_fight') {
					path = 'getBargainInfoFight'// 拼团
				}
				if(type === 'hx_exercise_time_limit') {
					path = 'getBargainInfoTime'// 限时秒杀
				}
				if(uid !== undefined) {
					type = type + '&uid=' + uid
				}
				uni.getStorage({
					...
					success: (res) => {
						this.token = JSON.parse(res.data);
						uni.request({
							url: `${getApp().globalData.URL}/clientExercise/${path}?id=${id}&type=${type}`,
							method: 'GET',
							header: {
								'Authorization': "Bearer " + this.token.token
							},
