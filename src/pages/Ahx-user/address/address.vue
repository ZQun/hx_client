<template>
	<view class="tui-safe-area">
			...
			<block v-for="(item,index) in addressList" :key="index">
				<tui-list-cell padding="0">
					<view class="tui-address-flex">
						<view @tap="goBack(item)" class="tui-address-left">
						...
						<view @tap="editAddress(item.id)" class="tui-address-imgbox">
		...
		<view class="tui-address-new">
			<tui-button type="green" height="88rpx" @click="addAddr">+ 新增收货地址</tui-button>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			addAddr(index, addressType) {
				uni.navigateTo({
					url: "/pages/Ahx-user/editAddress/editAddress"
				})
			},
			editAddress(id) {
				uni.navigateTo({
					url: `/pages/Ahx-user/editAddress/editAddress?id=${id}`
				})
			},
			goBack(data) {
			...
			getUserAddressList() {
				uni.getStorage({
					...
					success: (res) => {
						...
						uni.request({
							url: `${getApp().globalData.URL}/clientUser/getUserAddressList`,
							method: 'GET',
							header: {
